...
\midrule
\endhead
Internal operations dashboard & Supports merchant review, credential management, QR receiving account management, account administration, transaction investigation, reconciliation, audit inspection, and recovery actions. \\
\bodyrowrule
...
\bodyrowrule
Application services & Enforce merchant readiness, QR account readiness, payment and refund rules, QR generation, notification behavior, analytics preparation, reconciliation, and audit-aware state changes. \\
\bodyrowrule
...
\bodyrowrule
Merchant QR account & Active, inactive & Only one active QR account is allowed per merchant and QR provider, so payment QR generation has one authoritative receiving account. \\
\bodyrowrule
...
The project includes a worker entry point that runs the core asynchronous
control functions needed for time-based and retry-based behavior.

\begin{enumerate}
  \item A merchant payment request creates a pending payment with expiration
  metadata and VietQR payment evidence.
  ...
  \item The worker expires overdue pending payments and continues delivery
  attempts for due notifications.
...
inside focused application services, while the worker stays thin and
