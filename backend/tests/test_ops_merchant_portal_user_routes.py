...
class OpsMerchantPortalUserRouteTest(unittest.TestCase):
    def test_admin_can_list_create_update_and_reset_merchant_portal_users(self) -> None:
        ...
        override_current_internal_user(app, make_internal_user(role=InternalUserRole.ADMIN))

        try:
            with patch.object(
                ops_merchant_portal_user_controller.merchant_portal_user_admin_service,
                "list_users",
                ...
                list_response = TestClient(app).get("/v1/ops/merchants/m_demo/portal-users")
        ...
        self.assertEqual(list_response.json()["users"][0]["email"], "merchant@example.com")
        self.assertEqual(create_response.json()["generated_password"], "generated-secret")
        self.assertEqual(reset_response.json()["generated_password"], "reset-secret")
        self.assertIs(list_service.call_args.kwargs["db"], db)
        self.assertEqual(create_service.call_args.kwargs["merchant_id"], "m_demo")
        self.assertEqual(update_service.call_args.kwargs["status"], MerchantUserStatus.INACTIVE)
        self.assertEqual(reset_service.call_args.kwargs["merchant_id"], "m_demo")

    def test_ops_user_cannot_manage_merchant_portal_users(self) -> None:
        from app.main import app

        override_current_internal_user(app, make_internal_user(role=InternalUserRole.OPS))

        try:
            response = TestClient(app).get("/v1/ops/merchants/m_demo/portal-users")
        finally:
            app.dependency_overrides.clear()
