...
merchant_status = postgresql.ENUM("PENDING", "ACTIVE", "REJECTED", "SUSPENDED", "DISABLED", name="merchant_status", create_type=False)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.ForeignKeyConstraint(["approved_by"], ["internal_users.id"], name="fk_merchants_approved_by_internal_users"),
