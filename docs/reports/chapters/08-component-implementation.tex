...
Merchant-facing implementation is intentionally separate from merchant
integration access. Human merchant users sign in with a dedicated account model
and receive a merchant-scoped session. After sign-in, the gateway derives
merchant scope from that authenticated session rather than accepting merchant
...
The current system already includes the important control functions needed for
...
These functions are intentionally separated from ordinary request handling. The
result is a system in which payment state, refund state, notification delivery,
and operational recovery remain understandable as related but distinct
behaviors.
