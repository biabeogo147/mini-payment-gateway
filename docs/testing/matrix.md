...
| MER, ONB | `scenarios/merchant.md` |
| PAY | `scenarios/payment.md` |
| CB, EXP | `scenarios/callback.md` |
...
| ONB-01 | Ops registers merchant | Service, route, smoke | Phase 07 | Covered with DB seed | Yes | `backend/tests/test_merchant_ops_service.py`, `backend/tests/test_merchant_ops_routes.py`, `backend/scripts/smoke_ops_reconciliation_api.py` |
| ONB-02 | Ops submits onboarding case | Service, route, smoke | Phase 07 | Covered with DB seed | Yes | `backend/tests/test_merchant_ops_service.py`, `backend/tests/test_merchant_ops_routes.py`, `backend/scripts/smoke_ops_reconciliation_api.py` |
...
| REC-01 | Late success after expiration creates reconciliation evidence | Service, route | Phase 04, Phase 07 | Covered | Yes | `backend/tests/test_provider_callback_service.py`, `backend/tests/test_reconciliation_service.py`, `backend/tests/test_reconciliation_routes.py` |
| REC-02 | Callback amount mismatch creates reconciliation evidence | Service, route, smoke | Phase 04, Phase 07 | Covered with DB seed | Yes | `backend/tests/test_provider_callback_service.py`, `backend/tests/test_reconciliation_service.py`, `backend/tests/test_reconciliation_routes.py`, `backend/scripts/smoke_ops_reconciliation_api.py` |
...
| WH-10 | Ops manual retry sends failed event again | Route, service | Phase 06, Phase 07 | Covered | Yes | `backend/tests/test_webhook_delivery_service.py`, `backend/tests/test_webhook_ops_routes.py` |
| AUD-01 | Ops merchant action writes audit log | Service, smoke | Phase 07 | Covered with DB seed | Yes | `backend/tests/test_audit_service.py`, `backend/tests/test_merchant_ops_service.py`, `backend/scripts/smoke_ops_reconciliation_api.py` |
| AUD-02 | Credential rotation writes audit log | Service | Phase 07 | Covered | Yes | `backend/tests/test_audit_service.py`, `backend/tests/test_merchant_ops_service.py` |
| REC-03 | Matching provider evidence creates matched record | Service | Phase 07 | Covered | Yes | `backend/tests/test_reconciliation_service.py` |
| REC-04 | Status or amount mismatch creates mismatched record | Service | Phase 07 | Covered | Yes | `backend/tests/test_reconciliation_service.py` |
| REC-05 | Ops resolves reconciliation record | Service, route, smoke | Phase 07 | Covered with DB seed | Yes | `backend/tests/test_reconciliation_service.py`, `backend/tests/test_reconciliation_routes.py`, `backend/scripts/smoke_ops_reconciliation_api.py` |
| E2E-01 | Merchant onboarding to successful payment and refund | E2E | Phase 08 | Covered | Yes | `backend/tests/test_e2e_payment_refund_webhook.py::PaymentRefundWebhookE2ETests.test_happy_path_covers_onboarding_payment_refund_and_webhooks` |
| E2E-02 | Duplicate and idempotency path | E2E | Phase 08 | Covered | Yes | `backend/tests/test_e2e_payment_refund_webhook.py::PaymentRefundWebhookE2ETests.test_idempotent_payment_and_hmac_failure_are_explicit` |
| E2E-03 | Late callback reconciliation path | E2E | Phase 08 | Covered | Yes | `backend/tests/test_e2e_payment_refund_webhook.py::PaymentRefundWebhookE2ETests.test_late_success_callback_creates_reconciliation_case_and_ops_can_resolve_it` |
| E2E-04 | Webhook retry and manual retry path | E2E | Phase 08 | Covered | Yes | `backend/tests/test_e2e_payment_refund_webhook.py::PaymentRefundWebhookE2ETests.test_manual_webhook_retry_records_attempt_and_audit`, `backend/tests/test_e2e_payment_refund_webhook.py::PaymentRefundWebhookE2ETests.test_suspended_merchant_cannot_create_new_payments_or_refunds` |
