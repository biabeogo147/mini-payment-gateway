...
Use `docs/testing/scenarios/refund.md` as the behavior source for:
...
Use `docs/testing/scenarios/reconciliation.md` for refund callback mismatch behavior by
extending the same evidence-first pattern already used by phase 04.

Use `docs/testing/matrix.md` to keep the phase 05 test names aligned
...
- Modify: `docs/api/provider-callback.md`
...
- Create: `docs/history/completions/phase-05.md`
...
- [ ] Update `docs/api/merchant.md` if implementation fields differ from
  the contract above.
- [ ] Update `docs/api/provider-callback.md` so refund callback docs use
  `refund_transaction_id`, request status `SUCCESS`/`FAILED`, and response
  `refund_status`.
- [ ] Update `docs/api/errors.md` with
  `REFUND_INVALID_STATE_TRANSITION`.
- [ ] Update `docs/testing/scenarios/refund.md` statuses for implemented refund
  scenarios.
- [ ] Update `docs/testing/scenarios/reconciliation.md` with refund callback mismatch
...
- Refund rules match `docs/product/requirements.md` and `docs/testing/scenarios/refund.md`.
